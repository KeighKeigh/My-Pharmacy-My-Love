...
namespace ELIXIR.API.Controllers.ORDERING_CONTROLLER
{
    ...
    public class OrderingController : BaseApiController
    {
        ...
        public async Task<IActionResult> AddNewOrders([FromBody] Ordering[] order)
        {
            ...
            List<Ordering> notExistCustomer = new List<Ordering>();
            ...
            foreach (Ordering items in order)
            {
                ...
                //if (string.IsNullOrEmpty(items.AccountTitle))
                //{
                ...
                var validateCustomer = await _unitOfWork.Order.ValidateCustomer(items.FarmCode, items.FarmName);
                ...
                else if (validateCustomer == false)
                {
            ...
            if (notExistCustomer.Count == 0 && notExistFarmName.Count == 0 && notExistFarmCode.Count == 0 && notExistRawMats.Count == 0
                                    && notExistUom.Count == 0 && duplicateList.Count == 0 && previousdateNeeded.Count == 0
                                    //&& accountTitleEmpty.Count == 0
                                    && accountCodeEmpty.Count == 0)
            {
                await _unitOfWork.CompleteAsync();
