...
namespace ELIXIR.DATA.DATA_ACCESS_LAYER.REPOSITORIES.INVENTORY_REPOSITORY
{
    public class RawMaterialInventory : IRawMaterialInventory
    {
          ...
          var getSOH = (from warehouse in getWarehouseStock
            ...
            var getReserve = (from warehouse in getWarehouseStock
                              ...
                              select new ReserveInventory
                              {
                                  ...
                                  ItemCode = total.Key.ItemCode,
                                  Reserve = total.Sum(x => x.warehouse.ActualGood == null ? 0 : x.warehouse.ActualGood) -
                                    total.Sum(x => x.ordering.QuantityOrdered == null ? 0 : x.ordering.QuantityOrdered) - total.Sum(x => x.issue.Quantity == null ? 0 : x.issue.Quantity)
                              });
        ...
        public async Task<IReadOnlyList<MRPDtoWithItemCode>> GetAllItemForInventoryWithItemCode(string itemcode)
        {
            var EndDate = DateTime.Now;
            var StartDate = EndDate.AddDays(-30);

            var getPoSummary = _context.POSummary.Where(x => x.IsActive == true && x.ItemCode == itemcode)
              .GroupBy(x => new
              {
                  x.ItemCode,
            ...
            var getWarehouseIn = _context.WarehouseReceived.Where(x => x.IsActive == true && x.ItemCode == itemcode)
            .GroupBy(x => new
            {
                x.ItemCode,
            ...
            var getMoveOrderOut = _context.MoveOrders.Where(x => x.IsActive == true && x.ItemCode == itemcode)
                                                     .Where(x => x.IsPrepared == true)
                   .GroupBy(x => new
                   {
                       x.ItemCode,
            ...
            var getQCReceivingIn = _context.QC_Receiving.Where(x => x.ExpiryIsApprove == true)
                                                        .Where(x => x.IsActive == true && x.ItemCode == itemcode)
                   .GroupBy(x => new
                   {
                       x.ItemCode,
            ...
            var getReceiptIn = _context.WarehouseReceived.Where(x => x.IsActive == true && x.ItemCode == itemcode)
                                                         .Where(x => x.TransactionType == "MiscellaneousReceipt")
                   .GroupBy(x => new
                   {
                       x.ItemCode,
            ...
            var getIssueOut = _context.MiscellaneousIssueDetails.Where(x => x.IsActive == true && x.ItemCode == itemcode)
                                                                .Where(x => x.IsTransact == true)
                .GroupBy(x => new
                {
                    x.ItemCode,
            ...
            var getTransformation = _context.Transformation_Preparation.Where(x => x.IsActive == true && x.ItemCode == itemcode)
                .GroupBy(x => new
                {
                    x.ItemCode,
            ...
            var getWarehouseStock = _context.WarehouseReceived.Where(x => x.IsActive == true && x.ItemCode == itemcode)
             .GroupBy(x => new
             {
                 x.ItemCode,
            ...
            var getOrderingReserve = _context.Orders.Where(x => x.IsActive == true && x.ItemCode == itemcode)
                                                    .Where(x => x.PreparedDate != null)
           .GroupBy(x => new
           {
               x.ItemCode,
           ...
           }).Select(x => new OrderingInventory
           {
            ...
            var getTransformationReserve = _context.Transformation_Request.Where(x => x.IsActive == true && x.ItemCode == itemcode)
           .GroupBy(x => new
           {
               x.ItemCode,
           ...
           }).Select(x => new OrderingInventory
           {
            ...
            var getTransformTo = _context.WarehouseReceived.Where(x => x.IsActive == true && x.ItemCode == itemcode)
                                                           .Where(x => x.TransactionType == "Transformation")
              .GroupBy(x => new
