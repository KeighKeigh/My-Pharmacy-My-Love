...
namespace ELIXIR.DATA.DATA_ACCESS_LAYER.REPOSITORIES.SETUP_REPOSITORY
{
    public class CustomerRepository : GenericRepository<CustomerDto>, ICustomerRepository
    {
        ...
        public async Task<bool> UpdateCustomerInfo(Customer customer)
        {
            ...
            var existingCustomerName = await _context.Customers.FirstOrDefaultAsync(x => x.CustomerName == customer.CustomerName);

            if (existingCustomerName is not null)
            {
                return false;
            }
        ...
        public async Task<bool> CustomerNameExist(string customerName)
        {
            return await _context.Customers.AnyAsync(x => x.CustomerName == customerName);
        }
