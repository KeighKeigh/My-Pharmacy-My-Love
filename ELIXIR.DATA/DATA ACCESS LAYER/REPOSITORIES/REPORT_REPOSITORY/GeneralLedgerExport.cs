...
namespace ELIXIR.DATA.DATA_ACCESS_LAYER.REPOSITORIES.REPORT_REPOSITORY
{
    public class GeneralLedgerExport
    {
        ...
        public class Handler : IRequestHandler<GeneralLedgerExportCommand, Unit>
        {
            ...
            {
                var ledger = await _report.Report
                ...
                using (var workbook = new XLWorkbook())
                {
                    ...
                    for (var index = 1; index <= ledger.Count; index++)
                    {
                        ...
                        row.Cell(52).Value = ledger[index - 1].Transaction_Date != null
                            ? Convert.ToDateTime(ledger[index - 1].Transaction_Date).ToString("MMMM")
                            : "";
                        row.Cell(52).Style.Protection.SetLocked(false);
                        row.Cell(53).Value = ledger[index - 1].Transaction_Date != null
                            ? Convert.ToDateTime(ledger[index - 1].Transaction_Date).ToString("yyyy")
                            : "";
