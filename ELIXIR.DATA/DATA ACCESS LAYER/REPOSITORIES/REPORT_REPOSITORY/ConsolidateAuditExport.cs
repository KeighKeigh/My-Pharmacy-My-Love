...
namespace ELIXIR.DATA.DATA_ACCESS_LAYER.REPOSITORIES.REPORT_REPOSITORY
{
    ...
    {
        public class ConsolidateAuditCommand : IRequest<Unit>
        {
        ...
        public class Handler : IRequestHandler<ConsolidateAuditCommand, Unit>
        {
            ...
            public async Task<Unit> Handle(ConsolidateAuditCommand command, CancellationToken cancellationToken)
            {
                var consolidate = await _report.Report.ConsolidateAuditReport(command.DateFrom, command.DateTo, command.Search);
